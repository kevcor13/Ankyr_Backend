...
// Sub-schema for individual exercise entries within a workout segment
const ExerciseContentSchema = new mongoose.Schema({
    // The exact name of the exercise, matching `ExerciseLibrary.name`
    exercise: { type: String, required: true },
    sets: { type: Number, required: true },
    reps: { type: String, required: true },
    difficulty: { type: String, enum: ['easy', 'moderate', 'hard'], required: false },
    // The video URL is now directly stored here, fetched from ExerciseLibrary
    videoUrl: { type: String, required: true },
}, { _id: false }); // Do not create _id for sub-documents

const IndividualWorkoutSchema = new mongoose.Schema({
    // A unique hash representing the content of this workout segment.
    // This allows different users to share the same cached segment.
    contentHash: {
        type: String,
        required: true,
        unique: true,
...
const ExerciseLibrarySchema = new mongoose.Schema({
    // Unique identifier for the exercise (e.g., "Push-up", "Barbell Squat")
    // This MUST be exact for the AI to reference correctly.
    name: {
        type: String,
        required: true,
        unique: true,
    },
    // A detailed text description of how to perform the exercise correctly.
    description: {
        type: String,
        required: true,
    },
